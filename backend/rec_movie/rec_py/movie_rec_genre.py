...
def recommend(rec_g1, rec_g2, rec_g3):
    try:
        md = pickle.load(open("movie_genre_rec_df.pickle", "rb"))
        ...
        pickle.dump(md, open("movie_genre_rec_df.pickle", "wb"))
    ...
    qualified = md[(md['vote_count'] >= m) & (md['vote_count'].notnull()) & (md['vote_average'].notnull())][['movieId', 'original_title', 'year', 'vote_count', 'vote_average', 'popularity', 'genre']]
    ...
    result = []
    for movie_id in qualified['movieId']:

...
def recommend():
    try:
        md = pickle.load(open("movie_top_rec_df.pickle", "rb"))
        ...
        pickle.dump(md, open("movie_top_rec_df.pickle", "wb"))
    ...
    qualified = md[(md['vote_count'] >= m) & (md['vote_count'].notnull()) & (md['vote_average'].notnull())][['movieId', 'original_title', 'year', 'vote_count', 'vote_average', 'popularity', 'genres']]
